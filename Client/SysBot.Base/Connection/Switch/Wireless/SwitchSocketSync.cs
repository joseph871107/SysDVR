...
/// </remarks>
public class SwitchSocketSync(IWirelessConnectionConfig cfg) : SwitchSocket(cfg), ISwitchConnectionSync
{
    ...
    protected int Read(byte[] buffer, int size) => Connection.Receive(buffer, size, 0);
    ...
    protected static byte[] DecodeResult(ReadOnlyMemory<byte> buffer, int length)
    {
