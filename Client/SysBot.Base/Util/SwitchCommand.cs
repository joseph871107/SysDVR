...
public static class SwitchCommand
{
    ...
    private static string ToHex(short data) {
        return String.Format("{0}{1}", data < 0 ? "-" : "", $"0x{(data < 0 ? -data : data):X}");
    }
    ...
    public static byte[] SetStick(SwitchStick stick, short x, short y, bool crlf = true)
        => Encode(String.Format("setStick {0} {1} {2}", stick == SwitchStick.LEFT ? "LEFT" : "RIGHT", ToHex(x), ToHex(y)), crlf);
